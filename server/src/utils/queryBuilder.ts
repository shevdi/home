...
export const queryBuilder = () => {
  ...
  return {
    ...
    locationMatch(countries?: string[], cities?: string[], isFullSeaarch?: boolean) {
      ...
      if (countryList.length > 0) {
        const countryOrConditions = isFullSeaarch ? countryList.flatMap((countryTrim) => [
          { 'location.value.country': { $regex: countryTrim, $options: 'i' } },
        ...
        ]) : countryList.flatMap((countryTrim) => [
          { 'location.value.country': { $regex: countryTrim, $options: 'i' } },
        ])
        andConditions.push({ $or: countryOrConditions })
      }
      if (cityList.length > 0) {
        const cityOrConditions = isFullSeaarch ? cityList.flatMap((cityTrim) => [
          { 'location.value.city': { $regex: cityTrim, $options: 'i' } },
